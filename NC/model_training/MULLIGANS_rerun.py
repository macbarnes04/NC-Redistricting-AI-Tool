...
#
# Partisan Bias
partisan_bias_files = raw_data["PartisanBiasArray"]
partisan_bias_files = tf.convert_to_tensor(partisan_bias_files)
#
# Stacking / Scaling Metrics
#
scores = tf.stack([0.01*tf.cast(compactness_files, tf.float32), partisan_bias_files], axis=-1)
#
# --------------------------------------------------------------------------------------------
...
embed_ae.fit(x_train, x_train, epochs=10, batch_size=4)
...
encoder_conv_1 = tf.keras.layers.Conv2D(filters=32, kernel_size=3, strides=1, activation='relu')(embedded_encoder_input[...,None])
encoder_conv_2 = tf.keras.layers.Conv2D(filters=32, kernel_size=2, strides=1, activation='relu')(encoder_conv_1)
# output is 2703x1
flattened_encoder_embedding = tf.keras.layers.Flatten()(encoder_conv_2)
encoder_dense_1 = tf.keras.layers.Dense(1024, activation=tf.keras.activations.swish)(flattened_encoder_embedding)
encoder_dense_1_1 = tf.keras.layers.Dense(256, activation=tf.keras.activations.swish)(encoder_dense_1)
#
mean = tf.keras.layers.Dense(1024, activation=tf.keras.activations.swish)(encoder_dense_1_1)
logvar = tf.keras.layers.Dense(1024)(encoder_dense_1_1)
# the 128 is an arbitrary variable
metrics = tf.keras.layers.Dense(2, activation=tf.keras.activations.swish)(encoder_dense_1_1)
#
outputs = [
    mean,
    logvar,
...
decoder_dense_1 = tf.keras.layers.Dense(256, activation=tf.keras.activations.relu)(decoder_input)
decoder_dense_1_1 = tf.keras.layers.Dense(512, activation=tf.keras.activations.relu)(decoder_dense_1)
decoder_dense_2 = tf.keras.layers.Dense(1024, activation=tf.keras.activations.relu)(decoder_dense_1_1)
decoder_dense_2_2 = tf.keras.layers.Dense(1536, activation=tf.keras.activations.relu)(decoder_dense_2)
decoder_dense_3 = tf.keras.layers.Dense(2048, activation=tf.keras.activations.relu)(decoder_dense_2_2)
...
for epoch in range(epochs):
    ...
    for step, (images, metrics) in enumerate(train_dataset):
        ...
        epoch_accuracy_1.update_state(tf.equal(tf.math.argmax(images, axis=-1), tf.math.argmax(predictions[0], axis=-1)))
...
# --------------------------------------------------------------------------------------------
# SAVING MODEL
decoder.save("RI_mulligans_decoder.h5")
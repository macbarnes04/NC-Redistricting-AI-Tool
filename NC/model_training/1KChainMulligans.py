...
# IMPORT GRAPH DATA
mo_graph = Graph.from_file("/Users/macbarnes/PycharmProjects/GerryChain/mo_vtds.json")
...
# INITIAL PARTITION
initial_partition = GeographicPartition(mo_graph, assignment="Cong19Dist", updaters=updaters)
...
# FILING THE METRICS
total_nodes = len(mo_graph.nodes)
list_of_nodes = list(mo_graph.nodes)
cut_edge_array = []
county_splits_array = []
...
for part in chain:
    ...
    if int(counter / 10) == counter / 10:
        ...
        precinct_identifier = [mo_graph.nodes[y]["PREC_ID"] for y in range(total_nodes)]
...
with open("MO1000testrun1.json", "w") as data_file:
